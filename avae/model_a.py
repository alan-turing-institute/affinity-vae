...
class Encoder(nn.Module):
    def __init__(self, depth, filters, 
        latent_dims, pose_dims, unflat_shape, flat_shape):
        ...
        for d in range(len(filters)):
            self.encoder.append(conv(in_channels= input_channel , out_channels= filters[d], kernel_size=3, stride=2, padding=1))
...
class Decoder(nn.Module):
    ...
    def __init__(self,  depth, filters, latent_dims, pose_dims, unflat_shape, flat_shape):
        ...
        for d in reversed(range(len(filters))):
            if d !=0: 
                if not d==len(filters)-1:
                    self.decoder.append(conv_T(in_channels= filters[d], out_channels= filters[d-1], kernel_size=3, stride=2, padding=1))
                else:
                    self.decoder.append(conv_T(in_channels= filters[d], out_channels= filters[d-1], kernel_size=3, stride=2, padding=0))

                self.decoder.append(nn.ReLU(True))
        self.decoder.append(conv_T(in_channels= filters[0], out_channels= 1 , kernel_size=2, stride=2, padding=1))
...
class AffinityVAE(nn.Module):
    ...
    def __init__(
        ...
        self.unflat_shape = tuple(
            ...
            + [
                dims_after_pooling(ax, depth)
                for ax in input_size
            ]
        )
