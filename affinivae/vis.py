...
def format(im):
    ...
    if len(im.shape) == 5:
        batch = True
        im = np.sum(np.copy(im.squeeze(dim=1).cpu().detach().numpy()), axis=-1)  # .astype(np.uint8)
    elif len(im.shape) == 4:
        batch = False
...
def merge(im):
    ...
    i = im.split('&')
    if len(i) != 2:
        raise RuntimeError("Image format corrupt. Number of images in meta_df: {}".format(len(i)))
...
def vis_latentembed_plot(df, epoch, embedding='umap'):
    ...
    color = altair.condition(selection, altair.Color('id:N'), altair.value('lightgray'))
    chart = altair.Chart(df).mark_point(size=50, opacity=0.4).encode(
        ...
        #               title='dataset'),
        altair.Shape('mode', scale=altair.Scale(range=['square', 'circle', 'triangle']), title='mode'),
        altair.Tooltip(['id', 'meta', 'mode', 'avg', 'theta', 'image']),  # *degrees_of_freedom, 'image']),
        color=color
    ).interactive().properties(width=800, height=500).add_selection(selection)

    if not os.path.exists('plots/latent_embeds'):
        os.mkdir('plots/latent_embeds')
    if embedding == 'umap':
        chart.save(f"plots/latent_embeds/plt_latent_embed_epoch_{epoch}_umap.html")
    elif embedding == 'tsne':
        chart.save(f"plots/latent_embeds/plt_latent_embed_epoch_{epoch}_tsne.html")


def vis_loss_plot(epochs, loss, recon_loss=None, kldiv_loss=None, shape_loss=None, val_loss=None, p=None):
    print("\n####################################################################################")
    print("Visualising loss ...\n")

    plt.clf()
    plt.ticklabel_format(useOffset=False)
    plt.plot(range(1, epochs + 1), loss, c='blue', label='Total loss on training data')
    plt.plot(range(1, epochs + 1), recon_loss, c='red', label='Recon. loss on training data')
    plt.plot(range(1, epochs + 1), kldiv_loss, c='green', label='KL divergence loss x beta')
    plt.plot(range(1, epochs + 1), shape_loss, c='yellow', label='Shape similarity loss x gamma')
    plt.plot(range(1, epochs + 1), val_loss, c='black', label='Recon. loss on validation data')
    plt.yscale('log')
...
def vis_single_transversals(df, vae, device, patch_size, latent_dims, pose_dims, pose):
    ...
    if pose:
        ...
        mean_all_poses = [df_poses[lat_dim_col].mean() for lat_dim_col in df_poses.columns]
    ...
    for i in range(interp_recon.shape[0]):
        for j in range(interp_recon.shape[1]):
            grid_for_napari[i * patch_size[0]:(i + 1) * patch_size[0],
...
def vis_single_transversals_pose(df, vae, device, patch_size, latent_dims, pose_dims):
    print("\n####################################################################################")
    print("Visualising pose disentanglement ...\n")
    df_latent = df.filter(regex='lat').reset_index(drop=True)
    df_poses = df.filter(regex='pos').reset_index(drop=True)
    mean_all_poses = [df_poses[lat_dim_col].mean() for lat_dim_col in df_poses.columns]
    std_all_poses = [df_poses[lat_dim_col].std() for lat_dim_col in df_poses.columns]
    mean_all_dims = [df_latent[lat_dim_col].mean() for lat_dim_col in df_latent.columns]
    ...
    for pos_dim in range(pose_dims):
        for grid_spot in range(7):
            ...
            new_vector[pos_dim] += std_all_poses[pos_dim] * (-1.2 + 0.4 * grid_spot)
    ...
    interp_recon = np.reshape(np.array(interp_recon.cpu()), (pose_dims, 7, patch_size[0], patch_size[1], patch_size[2]))
...
def vis_interp_grid(df, vae, device, patch_size, pose):
    ...
    if len(class_rep_ids) <= 3:
        print("WARNING: Not enough distinct classes, cannot visualise interpolations.\n")
        return
    latents_of_class_reps = [df[df.id == pid].iloc[0].filter(regex='lat').values.astype(np.float32)
                             for pid in class_rep_ids]
    if pose:
        poses_of_class_reps = [df[df.id == pid].iloc[0].filter(regex='pos').values.astype(np.float32)
                               for pid in class_rep_ids]

    # Generate a gird of latent vectors interpolated between reps of four poke_ids
    grid_size = 6
    inds, class_rep_vecs = list(zip(*random.sample(list(enumerate(latents_of_class_reps)), k=4)))
    ...
    for i, corner in enumerate(corners):
        a, b = corner
        layers_l.append(np.where(1 - (np.sqrt((xx - a) ** 2 + (yy - b) ** 2)) > 0,
                                 1 - (np.sqrt((xx - a) ** 2 + (yy - b) ** 2)), 0)[np.newaxis, :, :])
    layers = np.concatenate([layers_l[0], layers_l[2], layers_l[1], layers_l[3]], axis=0)
